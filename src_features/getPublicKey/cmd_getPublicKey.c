...
void handleGetPublicKey(uint8_t p1, uint8_t p2, uint32_t dataBuffer,uint16_t dataLength, unsigned int *flags, unsigned int *tx) {
  ...
  io_seproxyhal_io_heartbeat();
  getAddressStringFromKey(&tmpCtx.publicKeyContext.publicKey, tmpCtx.publicKeyContext.address, &sha3);
#ifndef NO_CONSENT
  ...
  else
  {
    ...
    */
    ...
    snprintf(strings.common.fullAddress, sizeof(strings.common.fullAddress), "xdc%.*s", 41, tmpCtx.publicKeyContext.address);
